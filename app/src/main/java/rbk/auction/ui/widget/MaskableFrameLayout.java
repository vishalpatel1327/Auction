...
public class MaskableFrameLayout extends FrameLayout {
    ...
    private Drawable loadMask(TypedArray a) {
        return a.getDrawable(R.styleable.MaskableLayout_maskImage);
    }
    ...
    private Bitmap makeBitmapMask(@Nullable Drawable drawable) {
        if (drawable != null) {
            if (getMeasuredWidth() > 0 && getMeasuredHeight() > 0) {
                ...
                int left = (getMeasuredWidth() - size) / 2;
                int top = (getMeasuredHeight() - size) / 2;
                int right = size + (getMeasuredWidth() - size) / 2;
                int bottom = size + (getMeasuredHeight() - size) / 2;
